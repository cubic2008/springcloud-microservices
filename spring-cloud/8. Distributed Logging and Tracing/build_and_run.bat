...
echo ------------------
if not defined ZIPKIN_SERVER_JAR (
    set ZIPKIN_SERVER_JAR=..\software\zipkin-server-2.11.6-exec.jar
    echo "ZIPKIN_SERVER_JAR" is NOT defined, it is set to a default value.
) else (
    echo "ZIPKIN_SERVER_JAR" is defined.
)
echo ZIPKIN_SERVER_JAR = %ZIPKIN_SERVER_JAR%
start java -jar %ZIPKIN_SERVER_JAR%
timeout /T 3
