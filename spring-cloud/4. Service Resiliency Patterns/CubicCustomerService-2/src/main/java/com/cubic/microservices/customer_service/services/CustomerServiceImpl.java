	...
	@HystrixCommand	(fallbackMethod="retrieveNoCustomerProfiles",
		...
		commandProperties= {
			...
			@HystrixProperty(
					...
					value="false"),	// if set the value to "true", Hystrix will still call the rest methods (i.e., this.accountServiceDiscoveryClient.retrieveAccounts(customers)) in the getAllCustomers() method after the timeout occurs in the this.retrieveAllCustomerProfiles() method.
	} )
